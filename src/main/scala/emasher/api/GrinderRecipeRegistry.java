	...
	public static GrinderRecipe getRecipe( Object input ) {
		if( input instanceof ItemStack ) {
			...
			for( GrinderRecipe r : recipes ) {
				...
				if(  r.getInput() instanceof ItemStack && ( ( ItemStack ) input ).isItemEqual( ( ItemStack ) r.getInput() ) ) {
					if( r.getOutput() != null )
						return r;
				} else {
					for( int oreID : oreIDs ) {
						if( otherID == oreID && r.getOutput() != null ) {
							return r;
						}
					}
				}
			...
			for( GrinderRecipe r : recipes ) {
				...
				if( r.getInput() instanceof ItemStack ) {
					...
					int otherID = OreDictionary.getOreID( ( String ) r.getInput() );
					if( otherID != -1 && otherID == oreID && r.getOutput() != null ) {
						return r;
					}
				}
