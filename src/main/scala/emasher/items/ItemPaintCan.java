...
import emasher.blocks.BlockPipeBase;
...
public class ItemPaintCan extends Item {
	...
	public void registerIcons( IIconRegister registry ) {
		this.itemIcon = registry.registerIcon( "eng_toolbox:item" + ( 32 + paintColour ) );
	}
	...
	public ItemStack onItemRightClick( ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer ) {
		...
		if( var12 == null ) {
			...
			if( oreIDReq == oreIDThis ) {
				if( b != emasher.blocks.Blocks.paintedPlanks() || ( b == emasher.blocks.Blocks.paintedPlanks() && par2World.getBlockMetadata( var13, var14, var15 ) != this.paintColour ) ) {
					...
					par2World.setBlock( var13, var14, var15, emasher.blocks.Blocks.paintedPlanks(), paintColour, 2 );
			...
			if( b instanceof BlockPipeBase && par2World.getTileEntity( var13, var14, var15 ) != null && par2World.getTileEntity( var13, var14, var15 ) instanceof TilePipeBase ) {
				if( !par2World.isRemote ) {
					...
					EngineersToolbox.network().sendToDimension(new PipeColourMessage((TilePipeBase) par2World.getTileEntity(var13, var14, var15)), par2World.provider.dimensionId);
				}
				par2World.playSoundEffect( ( double ) var13 + 0.5D, ( double ) var14 + 0.5D, ( double ) var15 + 0.5D, "step.cloth", 1.0F, itemRand.nextFloat() * 0.4F + 0.8F );
				for( int i = 0; i < 16; i++ ) {
					int met = 15 - this.paintColour;
