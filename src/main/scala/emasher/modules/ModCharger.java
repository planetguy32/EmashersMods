...
public class ModCharger extends SocketModule {
	...
	public void getIndicatorKey( List l ) {
		l.add( emasher.util.Config.PREF_GREEN() + "Inventory containing item to charge" );
		l.add( emasher.util.Config.PREF_AQUA() + "Item dependent power requirement" );
	}
	...
	public void addRecipe() {
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), "ggg", "slr", " b ", Character.valueOf( 'g' ), Blocks.glass_pane, Character.valueOf( 's' ), Items.glowstone_dust,
				Character.valueOf( 'l' ), "dyeLime", Character.valueOf( 'r' ), Items.gold_ingot, Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
	}
	...
	public IIcon[] getAdditionalOverlays( SocketTileAccess ts, SideConfig config, ForgeDirection side ) {
		return new IIcon[] {( ( BlockSocket ) emasher.blocks.Blocks.socket() ).chargeInd[config.meta]};
	}
