...
public class ModFracker extends SocketModule {
	...
	public void getIndicatorKey( List l ) {
		...
		l.add( emasher.util.Config.PREF_GREEN() + "Output tank" );
	...
	public void addRecipe() {
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), "hih", "cdu", " b ", Character.valueOf( 'h' ), Blocks.hopper, Character.valueOf( 'u' ), Items.bucket,
				Character.valueOf( 'i' ), Items.iron_ingot, Character.valueOf( 'c' ), emasher.items.Items.circuit(), Character.valueOf( 'd' ), Items.diamond, Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
		
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), "hih", "cdu", " b ", Character.valueOf( 'h' ), Blocks.hopper, Character.valueOf( 'u' ), Items.bucket,
				Character.valueOf( 'i' ), "ingotAluminum", Character.valueOf( 'c' ), emasher.items.Items.circuit(), Character.valueOf( 'd' ), Items.diamond, Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
		
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), "hih", "cdu", " b ", Character.valueOf( 'h' ), Blocks.hopper, Character.valueOf( 'u' ), Items.bucket,
				Character.valueOf( 'i' ), "ingotTin", Character.valueOf( 'c' ), emasher.items.Items.circuit(), Character.valueOf( 'd' ), Items.diamond, Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
	}
	...
	public void onTankChange( SideConfig config, int index, SocketTileAccess ts, ForgeDirection side, boolean add ) {
		if( index == config.tank && add && side == ForgeDirection.DOWN && config.inventory >= 0 && config.inventory < 3 ) {
			...
			if( f.isFluidEqual( new FluidStack( emasher.fluids.Fluids.fluidSlickwater(), 1000 ) ) || f.isFluidEqual( new FluidStack( FluidRegistry.WATER, 1000 ) ) ) {
				
				int amntToDraw;
				boolean sw = false;
				
				if( f.isFluidEqual( ( new FluidStack( emasher.fluids.Fluids.fluidSlickwater(), 1000 ) ) ) ) {
					amntToDraw = 1000;
					sw = true;
				} else {
					amntToDraw = 200;
				...
				if( ts.getWorldObj().getBlock( ts.xCoord, ts.yCoord - 1, ts.zCoord ) == emasher.blocks.Blocks.shaleResource() && ts.getWorldObj().getBlockMetadata( ts.xCoord, ts.yCoord - 1, ts.zCoord ) != 2 ) {
