...
import emasher.modules.Orientation.Orientation
...
class ModLazySusan( id: Int ) extends SocketModule( id, "eng_toolbox:lazySusan" ) {
  ...
  override def addRecipe( ): Unit = {
    CraftingManager.getInstance( ).getRecipeList.asInstanceOf[ java.util.List[ Object ] ]
      .add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module, 4, moduleID ), "eme", "mpm", "eme",
      Character.valueOf( 'p' ), Items.iron_ingot,
      Character.valueOf( 'm' ), new ItemStack( emasher.items.Items.module, 1, 40 ),
      Character.valueOf( 'e' ), new ItemStack( emasher.items.Items.gem, 1, 0 ) ) )
  }
  ...
  override def onRSInterfaceChange( config: SideConfig, index: Int, ts: SocketTileAccess, side: ForgeDirection, on: Boolean ): Unit = {
    ...
    if( ts.getWorldObj( ).getBlock( one.x, one.y, one.z ) == Blocks.miniPortal ) return
    if( ts.getWorldObj( ).getBlock( two.x, two.y, two.z ) == Blocks.miniPortal ) return
    if( ts.getWorldObj( ).getBlock( three.x, three.y, three.z ) == Blocks.miniPortal ) return
    if( ts.getWorldObj( ).getBlock( four.x, four.y, four.z ) == Blocks.miniPortal ) return
