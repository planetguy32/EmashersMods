...
public class ModMultiSmelter extends SocketModule {
	...
	public ModMultiSmelter( int id ) {
		super( id, "eng_toolbox:multiSmelter" );
	}
	...
	public void getIndicatorKey( List l ) {
		l.add( emasher.util.Config.PREF_BLUE() + "Input inventory 1" );
		l.add( emasher.util.Config.PREF_GREEN() + "Input inventory 2" );
		l.add( emasher.util.Config.PREF_YELLOW() + "Outputs to Machine Output" );
		l.add( emasher.util.Config.PREF_AQUA() + "Requires 20 RF/t" );
		l.add( "Cannot be installed on a socket with other machines" );
	}
	
	@Override
	public void addRecipe() {
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), " h ", "uiu", " b ", Character.valueOf( 'i' ), "blockNickel", Character.valueOf( 'h' ), emasher.items.Items.psu(),
				Character.valueOf( 'u' ), Blocks.furnace, Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
	}
	...
	public String[] getAllInternalTextures() {
		return new String[] {"eng_toolbox:inner_fire_orange"};
	}
