...
public class ModSpinningWheel extends SocketModule {
	...
	public void addRecipe() {
		CraftingManager.getInstance().getRecipeList().add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module(), 1, moduleID ), " hs", "sws", "sb ", Character.valueOf( 's' ), Items.stick, Character.valueOf( 'h' ), emasher.items.Items.psu(),
				Character.valueOf( 'w' ), "plankWood", Character.valueOf( 'b' ), emasher.items.Items.blankSide() ) );
	}
		...
		if( config.inventory >= 0 && config.inventory <= 2 ) {
			if( ts.sideInventory.getStackInSlot( side.ordinal() ) == null ) {
				if( ts.getStackInInventorySlot( config.inventory ) != null ) {
					...
					if( toIntake.getItem() == Item.getItemFromBlock( Blocks.wool ) )
						product = new ItemStack( Items.string, 4, 0 );
