...
import emasher.util.{Coords, FrameMovementUtil}
...
class ModTrack( id: Int ) extends SocketModule( id, "eng_toolbox:trackUp", "eng_toolbox:trackLeft", "eng_toolbox:trackDown", "eng_toolbox:trackRight" ) {
  ...
  override def addRecipe( ): Unit = {
    CraftingManager.getInstance( ).getRecipeList.asInstanceOf[ java.util.List[ Object ] ]
      .add( new ShapedOreRecipe( new ItemStack( emasher.items.Items.module, 2, moduleID ), "pmp", "iii",
      Character.valueOf( 'i' ), new ItemStack( emasher.items.Items.gem, 1, 0 ),
  ...
  override def getIndicatorKey( l: java.util.List[ Object ] ): Unit = {
    ...
    l.add( emasher.util.Config.PREF_WHITE + "Change Direction" )
  }
  ...
  override def onRSInterfaceChange( config: SideConfig, index: Int, ts: SocketTileAccess, side: ForgeDirection, on: Boolean ): Unit = {
    ...
    if( canMove ) {
      ...
      val y = ts.yCoord
      val z = ts.zCoord
      val done = FrameMovementUtil.moveGroup( world, Coords( x, y, z ), dir )
