...
class PhotobioreactorRecipeHandler extends BaseRecipeHandler {
  final val recipeId = "sockets.photobioreactor"

  override def getRecipeName: String = StatCollector.translateToLocal( "item.socket_module.101.name" ) + " Module"

  override def getGuiTexture: String = "eng_toolbox:textures/gui/nei-photobioreactor.png"

  case class CachedPhotobioreactorRecipe( input: List[Object], output: ItemStack, inputFluidAmount: Int, outputFluidAmount: Int ) extends CachedRecipe {
  ...
  def makeCached( recipe: PhotobioReactorRecipe ): Option[CachedPhotobioreactorRecipe] = {

    if( recipe.getInput != null && recipe.getFluidInput != null && recipe.getOutput != null ) {
      val input1 = extractItemStackList( recipe.getInput )
      if( input1.nonEmpty ) {
        Option( CachedPhotobioreactorRecipe( List( recipe.getInput, new ItemStack( recipe.getFluidInput.getFluid.getBlock ) ),
  ...
  override def loadCraftingRecipes( result: ItemStack ): Unit = {
    PhotobioReactorRecipeRegistry.recipes.toList.filter { recipe =>
  ...
  override def loadCraftingRecipes( id: String, result: Object* ): Unit = {
    if( id == recipeId && this.getClass == classOf[PhotobioreactorRecipeHandler] ) {
      PhotobioReactorRecipeRegistry.recipes.toList.flatMap( makeCached ).filter { cached =>
  ...
  override def drawExtras( recipe: Int ): Unit = {
    val cachedRecipe = arecipes.get( recipe ).asInstanceOf[CachedPhotobioreactorRecipe]
